...
STUDY_DB = "sqlite:///" + str(ROOT / "outputs" / "opt_hw2.db")
PI = math.pi
...
def objective(trial, duration):
    ...
    cfg = apply_overrides(_CFG, [
        "hardware.body_amp=%g"  % trial.suggest_float("body_amp", 0.3, 1.3),
        "hardware.leg_amp=%g"   % trial.suggest_float("leg_amp", 0.3, 1.6),
